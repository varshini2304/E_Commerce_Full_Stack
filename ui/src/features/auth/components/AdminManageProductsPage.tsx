  ...
  const rows: ProductRow[] = useMemo(
    () =>
      products.slice(0, 7).map((product, index) => {
        const stockValues = [24, 15, 32, 8, 5, 20, 51];
        const stock = stockValues[index] ?? 12;
        return {
          id: `${product.id}-${index}`,
          product,
          stock,
          ...
          <div className="grid gap-6 lg:grid-cols-[240px_1fr]">
            ...
            <section className="space-y-4">
              <header className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-[#e2e7fb] bg-white/85 px-6 py-4">
                <h1 className="text-5xl font-semibold text-[#2b3869]">Manage Products</h1>
                <button
              ...
              <section className="overflow-hidden rounded-2xl border border-[#dfe5fb] bg-white/90">
                <div className="grid grid-cols-[2fr_1fr_80px_1fr_100px] gap-3 border-b border-[#e9ecfa] bg-[#f4f6ff] px-4 py-3 text-2xl text-[#5f6a94]">
                  <span>Image</span>
                  <span>Price</span>
                  <span>Stock</span>
                  <span>Status</span>
                ...
                {rows.map((row) => (
                  <div
                    className="grid grid-cols-[2fr_1fr_80px_1fr_100px] items-center gap-3 border-b border-[#eef1fb] px-4 py-3 last:border-0"
                    key={row.id}
                  >
                    ...
                    <p className="text-3xl text-[#2e3b6d]">{row.stock}</p>
                    <span
