...
interface InventoryRow {
  ...
  product: ProductData;
  stockLabel: string;
  alertLevel: "below5" | "below10" | "ok";
}

const alertClassMap = {
  below5: "bg-[#f3dfc5] text-[#9e6c2f]",
  below10: "bg-[#cce2ea] text-[#2f5f77]",
  ok: "bg-[#cbe7db] text-[#2f725b]",
} as const;
  ...
  const rows: InventoryRow[] = useMemo(
    () =>
      products.slice(0, 8).map((product, index) => {
        ...
        const isLow = index >= 4 && index < 8;

        return {
          id: product.id,
          product,
          stockLabel: isCritical ? "Below 5" : isLow ? "Below 10" : "Delivered",
          alertLevel: isCritical ? "below5" : isLow ? "below10" : "ok",
        };
      }),
    [products],
  );
          ...
          <div className="grid gap-6 lg:grid-cols-[240px_1fr]">
            ...
            <section className="space-y-4">
              <header className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-[#e2e7fb] bg-white/85 px-6 py-4">
                <h1 className="text-5xl font-semibold text-[#2b3869]">Inventory Management</h1>
                <button
              ...
              <section className="rounded-2xl border border-[#e2e7fb] bg-white/90 p-5">
                <div className="rounded-xl border border-[#f2dec6] bg-[#fdf0e2] px-5 py-4">
                  <h2 className="text-4xl font-semibold text-[#7a5d38]">Low Stock Alerts</h2>
                  <p className="mt-1 text-2xl text-[#8a7050]">
                ...
                <div className="mt-4 flex flex-wrap gap-2">
                  <button
                    className="h-10 rounded-lg border border-[#d5dbf7] bg-[#4362c8] px-5 text-2xl text-white"
                    type="button"
                  >
                    Restock Selected
                  </button>
                  <button
                    className="h-10 rounded-lg border border-[#d5dbf7] bg-white px-5 text-2xl text-[#6a759f]"
                    type="button"
                  >
                    Bulk Update
                  </button>
              ...
              <section className="overflow-hidden rounded-2xl border border-[#dfe5fb] bg-white/90">
                <div className="grid grid-cols-[40px_1.5fr_1fr_1fr_140px] gap-3 border-b border-[#e9ecfa] bg-[#f4f6ff] px-4 py-3 text-2xl text-[#5f6a94]">
                  <span />
                  <span>Product</span>
                  <span>Stock</span>
                ...
                {rows.map((row) => (
                  <div
                    className="grid grid-cols-[40px_1.5fr_1fr_1fr_140px] items-center gap-3 border-b border-[#eef1fb] px-4 py-3 last:border-0"
                    key={row.id}
                  >
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      />
                      <span className="text-2xl text-[#2f3c6d]">{row.product.name}</span>
                    </div>
                    <span className="text-3xl text-[#2e3b6d]">
                      {formatPrice(row.product.price, row.product.currency)}
                    </span>
                    <span
                      className={`w-fit rounded-full px-4 py-1 text-xl ${alertClassMap[row.alertLevel]}`}
                    >
                      {row.stockLabel}
                    </span>
                    <button
                      className="h-10 rounded-lg bg-[#4362c8] px-4 text-2xl font-medium text-white"
                      type="button"
                    >
                      Restock
                    </button>
                ...
                <div className="px-4 py-4 text-right text-xl text-[#7a85ae]">
                  Showing 1 to {Math.min(5, rows.length)} of {rows.length} entries
                </div>
              </section>
            </section>
          </div>
